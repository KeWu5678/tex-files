...
With Taylor we can linearize the objective function and constraints to get an alternative (linearized) form of the feasible direction: 
\begin{definition}[linearized feasible directions]
\end{definition}

However, this only holds if the linearized structure really capture the geometric structure around the feasible set: 


\section{Equality Constrained Optimization}
\subsection{the constraints qualification}
...
\begin{proposition}
	Given $X \subset \mathbb{R}^n$ open and $c$ a regular value, $T_p M^\perp$ admits the basis
	\begin{equation}
		\mathcal{B} = \{ \nabla f^1(p) \dots \nabla f^l(p) \}. 
	\end{equation}
\end{proposition}
...
\subsubsection{the naive Newton method}
\textbf{Questions}: 
\begin{enumerate}
	\item what is the null space method
\end{enumerate}


\subsubsection{the penalty approaches}
\textbf{Questions}:
\begin{enumerate}
...
\begin{equation}
	\begin{aligned}
		& \min_d \langle \nabla F(x+k), d \rangle + \langle d, H d \rangle\\
		& \text{s.t.}\\
...
Notice that the subgradient, it is not only defined in a neighborhood of $x$ but globally. The following theorem states, for the convex function, the relationship between the Gateau derivative and the sub-gradient exists. 
...
\chapter{Nonsmooth Optimization}
...
\begin{definition}[Star-shaped area]
\end{definition}
...
\begin{theorem}
\textbf{Given a convex function, a point is a minimum point iff 0 is included in the subdifferential:} For a convex function defined on a non-empty, open and convex set, the following arguments are equivalent:
	\begin{enumerate}
...
\textbf{Locally Lipschitz continuous function is Clark differentiable in the point}. And we have:
\begin{equation*}
...
\begin{theorem}
	\textbf{Locally Lipschitz continuous function admit in every point the Clark sub-derivative, and the sub-derivative} and:
	\begin{enumerate}
...
\textbf{Local Lipschitz continuous functions are a.e. differentiable.}
\end{theorem}
...
\subsubsection*{The evaluation procedure}

\section{Subgradient Methods}
\subsection{introduction}
gradient ist immer eine abstiegrichtung. Aber sub-gradient nicht. 
...
\begin{definition}[sub-gradient, subdifferential]
	Given $f: D \longrightarrow \mathbb{R}$ convex on a convex, open subset of $\mathbb{R}^n$. A vector is called sub-gradient of $f$ in the point $\hat{x}$, if
	\begin{equation}
...
\begin{remark}\quad
	\begin{enumerate}
		\item We would expect the one sub-gradient $g \in \partial f(x)$ can be calculated in every step $\Rightarrow$ Oracle.
		\item The sub-gradient method is not a descent method.
	\end{enumerate}
\end{remark}


\begin{lemma}
	$f : \mathbb{R}^n \longrightarrow \mathbb{R}$ a convex function. $\tilde{x}$ an arbitrary minimal point of $f$ and $x^k$ the k-th iteration of the sub-gradient method and not the minimal point. Then there exists $\tilde{\alpha} > 0$ , s.t.for $x^{k + 1} = x^{k} + \alpha_k d^k$ we have:
		\begin{equation*}
...
\begin{proof}
	By the sub-gradient method we get:
	\begin{equation*}
...
\begin{proof}
	...
	\begin{enumerate}
		...
		\item By the definition of the sub-gradient we have:
			\begin{equation*}
...
\begin{remark}\quad
	\begin{enumerate}
		\item We mainly apply the Lagrange and the definition of the sub-gradient.
		\item Notice that the sub-gradient method is not a descent method.
	\end{enumerate}
\end{remark}
...
\begin{theorem}
	...
	\end{equation*}
	then the sub-gradient method will not stop in finite steps but converges to the minimal point:
	\begin{equation*}
...
By the definition of sub-gradient we know that $f(x)$ is lower bounded by the function:
\begin{equation} 
...
\begin{algorithmthm}
	...
	\begin{enumerate}
		\item calculate a sub-gradient for each point $g^k \in \partial f(x^k)$
